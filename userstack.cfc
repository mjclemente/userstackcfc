...
*/
component displayname="userstackcfc"  {

    variables._userstackcfc_version = '0.0.0';

    public any function init(
        string access_key = '',
        string baseUrl = "https://api.userstack.com",
        ...
        var secrets = {
            'access_key': 'USERSTACK_ACCESS_KEY'
        };
    ...
    /**
    * @docs https://userstack.com/documentation#single_lookup
    * @hint Lookup a single user-agent
    * @ua specifies the User-Agent string to be looked up.
    * @fields [optional] can be used to specify API response field(s).
    */
    public struct function detect( required string ua, string fields, string callback, string output ) {
      var params = {
        "ua": ua
      };
      if( !isNull(fields) ){
      ...
      return apiCall( 'GET', '/detect', params );
    }
    ...
    private struct function getBaseHttpHeaders() {
        return {
            ...
            'User-Agent' : 'userstackcfc/#variables._userstackcfc_version# (ColdFusion)'
        };
    }
